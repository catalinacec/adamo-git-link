...
import { UserMapper } from "../middlewares/mappers/user.mapper";
...
export class UserRepository implements IUserRepository {
  ...
  async findByUUID(uuid: string): Promise<User | null> {
    return await this.findOneAndMap({ uuid });
  }
  ...
  async findAllActive(): Promise<User[]> {
    ...
    return users.map((user) => UserMapper.toDomain(user as IUserModel));
  }
}
